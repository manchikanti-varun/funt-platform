...
export async function getBatchCourseForStudent(studentId: string, batchId: string, courseId?: string) {
  ...
  return {
    ...
    courseSnapshot: {
      ...
      deliveryMode: (snapshot as { deliveryMode?: string }).deliveryMode,
      enrollmentPriceInPaise: (snapshot as { enrollmentPriceInPaise?: number }).enrollmentPriceInPaise ?? 0,
      learningPlan: (snapshot as { learningPlan?: unknown }).learningPlan,
      modules,
      chapters: modules,
    },
...
function computeProgressPercentFromDocs(
  ...
  progressDocs: Array<{ moduleOrder: number; completedAt?: Date | null; contentCompletedAt?: Date | null; videoCompletedAt?: Date | null; youtubeCompletedAt?: Date | null; assignmentCompletedAt?: Date | null; quizCompletedAt?: Date | null }>
): number {
