  ...
  return (
    <AppPageShell className="gap-4">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
        <PageSection className="border-indigo-200 bg-gradient-to-br from-indigo-50/50 to-white shadow-md">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-indigo-700">Today Mission</p>
              <h1 className="mt-1 text-xl font-semibold text-black">{missionText}</h1>
              <p className="mt-1 text-sm text-black/65">{focusQuestion}</p>
            </div>
            <div className="inline-flex items-center gap-1 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-700">
        ...
        <PageSection className="border-indigo-200 bg-white">
          <div className="grid grid-cols-3 gap-2">
            <div className="rounded-xl border border-indigo-200 bg-white px-3 py-2.5">
              <p className="text-[11px] uppercase tracking-[0.12em] text-indigo-700">XP</p>
              <p className="mt-1 text-lg font-semibold text-black">{xp}</p>
            </div>
            <div className="rounded-xl border border-indigo-200 bg-white px-3 py-2.5">
              <p className="text-[11px] uppercase tracking-[0.12em] text-indigo-700">Level</p>
              <p className="mt-1 text-lg font-semibold text-black">{level}</p>
            </div>
            <div className="rounded-xl border border-indigo-200 bg-white px-3 py-2.5">
              <p className="text-[11px] uppercase tracking-[0.12em] text-indigo-700">Pace</p>
              <p className="mt-1 text-lg font-semibold text-black">{avgProgress}%</p>
            </div>
          </div>
          <div className="mt-3 h-2 rounded-full bg-indigo-100">
            <div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-indigo-600" style={{ width: `${xpProgress}%` }} />
          </div>
        ...
        <PageSection title="Attention" subtitle="Handle first" className="border-indigo-200 bg-white">
          <div className="space-y-2">
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
              <p className="inline-flex items-center gap-1.5 text-xs font-semibold text-black"><CalendarClock className="h-3.5 w-3.5 text-indigo-700" aria-hidden /> Pending reviews</p>
              <p className="mt-1 text-sm text-black">{pendingReviews}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
              <p className="inline-flex items-center gap-1.5 text-xs font-semibold text-black"><Lock className="h-3.5 w-3.5 text-indigo-700" aria-hidden /> Blocked courses</p>
              <p className="mt-1 text-sm text-black">{blocked.length}</p>
            </div>
            <Link href="/courses" className="inline-flex items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700">Resolve <ArrowRight className="h-3.5 w-3.5" aria-hidden /></Link>
          </div>
        </PageSection>

        <PageSection title="Journey" subtitle="Now / Risk / Next" className="border-indigo-200 bg-white">
          <div className="space-y-2">
            <div className="rounded-xl border border-indigo-200 bg-white px-3 py-2.5">
              <p className="text-[11px] uppercase tracking-[0.12em] text-indigo-700">Now</p>
              <p className="mt-0.5 text-sm font-semibold text-black">{nextCourse ? nextCourse.courseTitle : "Start first course"}</p>
            </div>
            <div className="rounded-xl border border-indigo-200 bg-white px-3 py-2.5">
              <p className="text-[11px] uppercase tracking-[0.12em] text-indigo-700">Risk</p>
              <p className="mt-0.5 text-sm font-semibold text-black">{trailingCourse ? trailingCourse.courseTitle : "No lag detected"}</p>
            </div>
            <div className="rounded-xl border border-indigo-200 bg-white px-3 py-2.5">
              <p className="text-[11px] uppercase tracking-[0.12em] text-indigo-700">Next unlock</p>
              <p className="mt-0.5 text-sm font-semibold text-black">Level {level + 1}</p>
            </div>
          </div>
        </PageSection>

        <PageSection title="Cohort Pulse" subtitle="What’s moving" className="border-indigo-200 bg-white">
          <div className="space-y-2">
            <div className="rounded-xl border border-indigo-200 bg-white px-3 py-2.5">
              <p className="inline-flex items-center gap-1.5 text-xs font-semibold text-black"><Users className="h-3.5 w-3.5 text-indigo-700" aria-hidden /> Active enrollments</p>
              <p className="mt-1 text-sm text-black">{active.length}</p>
            </div>
            <div className="rounded-xl border border-indigo-200 bg-white px-3 py-2.5">
              <p className="inline-flex items-center gap-1.5 text-xs font-semibold text-black"><Target className="h-3.5 w-3.5 text-indigo-700" aria-hidden /> Focus call</p>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.3fr)_minmax(0,0.7fr)]">
        <PageSection title="Momentum Board" subtitle="Top active courses" className="border-indigo-200 bg-white">
          <div className="grid gap-2">
            {sortedByProgress.slice(0, 4).map((c) => (
              <Link key={`${c.batchId}-${c.courseId}`} href={`/courses/${c.courseId}?batchId=${c.batchId}`} className="rounded-xl border border-indigo-200 bg-white px-3 py-2.5 transition hover:border-indigo-400">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm font-semibold text-black">{c.courseTitle}</p>
                  <p className="text-xs font-semibold text-indigo-700">{c.progressPercent}%</p>
                </div>
        ...
        <PageSection title="Unlocked" subtitle="Recent rewards" className="border-indigo-200 bg-white">
          <div className="space-y-2">
            {recentUnlocks.length === 0 ? (
              <p className="text-sm text-black/55">No recent unlocks yet.</p>
            ) : (
              recentUnlocks.map((a) => (
                <div key={a.id} className="rounded-xl border border-indigo-200 bg-white px-3 py-2.5">
                  <p className="inline-flex items-center gap-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-indigo-700">
      ...
      <PageSection title="Recommended Next Paths" subtitle="Join a new cohort lane" className="border-indigo-200 bg-white">
