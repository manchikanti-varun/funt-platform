  ...
  useEffect(() => {
    Promise.all([
      api<MyCourse[]>("/api/student/courses").then((r) =>
        r.success && Array.isArray(r.data) ? r.data.filter((c) => c.batchType === "online") : []
      ).catch(() => [] as MyCourse[]),
      api<ExploreCourse[]>("/api/student/courses/explore").then((r) =>
        r.success && Array.isArray(r.data) ? r.data.filter((c) => c.batchType === "GLOBAL_ONLINE") : []
    ...
    <AppPageShell className="flex flex-col gap-5">
      <div className="page-hero flex shrink-0 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Complete course + brand new kit. One-time purchase — the kit is yours to keep forever.</p>
        </div>
        ...
        <DataPanel className="flex flex-col bg-white/95">
          <div className="flex flex-1 items-center justify-center p-10">
            <p className="text-sm text-slate-500">
              {searchQuery.trim() ? "No courses match your search." : "No online courses available yet."}
            </p>
          </div>
        </DataPanel>
      )}
