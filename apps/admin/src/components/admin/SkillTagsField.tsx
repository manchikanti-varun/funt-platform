      ...
      {customSelected.length > 0 ? (
        <div className="flex flex-wrap gap-2">
          {customSelected.map((tag) => (
            ...
            >
              {tag}
              <span className="text-slate-400"><svg className="inline h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></span>
            </button>
          ))}
        </div>
      ) : null}
