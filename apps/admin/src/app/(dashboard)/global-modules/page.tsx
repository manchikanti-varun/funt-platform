...
export default function GlobalChaptersPage() {
  ...
  useEffect(() => { setCurrentPage(1); }, [debouncedSearch, selectedCourseId]);
                  ...
                  {isSuperAdmin && (
                    <th className="w-10 px-3 py-4">
                      <input
                        ...
                        onChange={(e) =>
                          setSelectedIds(e.target.checked ? new Set(paginatedList.map((m) => m.id)) : new Set())
                        }
          ...
          {sortedList.length > pageSize && (
            <div className="flex flex-wrap items-center justify-between gap-4 border-t border-slate-200 px-5 py-3">
              <div className="flex items-center gap-4 text-sm text-slate-600">
                <span>{sortedList.length} chapter{sortedList.length !== 1 ? "s" : ""}</span>
