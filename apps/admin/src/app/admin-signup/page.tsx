...
function AdminSignupForm() {
  ...
  const [roleType, setRoleType] = useState<"ADMIN" | "SUPER_ADMIN">("ADMIN");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok && data?.data?.user) {
        ...
        setSubmittedMessage(
          data.message ??
            "Profile submitted successfully. Your request is pending Super Admin approval. You can sign in only after approval."
        );
      } else {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-50 via-white to-teal-50/40 p-4">
        <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-soft text-center">
          ...
          <p className="mt-2 text-xs text-slate-500">Contact an existing Super Admin to review your request.</p>
          <button
            type="button"
            onClick={() => router.replace("/login")}
        ...
        {step === 1 ? (
          <form onSubmit={handleStep1Next} className="mt-6 space-y-4">
            ...
            <div>
              ...
              <select
                value={roleType}
                onChange={(e) => setRoleType(e.target.value === "SUPER_ADMIN" ? "SUPER_ADMIN" : "ADMIN")}
                className="input w-full"
              >
